...
def add_number_to_gear(i, j, number):
    if gears.get((i, j)):
    ...
    for number in numbers:
        ...
        if i - 1 >= 0:
            for elem in schematic[i - 1][number_span[0] - 1 : number_span[1] + 1]:
                if elem == "*":
                    add_number_to_gear(i - 1, schematic[i - 1].index(elem, number_span[0] - 1), number)
        if line[number_span[0] - 1] == "*":
            add_number_to_gear(i, number_span[0] - 1, number)
        if line[number_span[1]] == "*":
            add_number_to_gear(i, number_span[1], number)
